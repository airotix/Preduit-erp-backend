...
read = require_module("ai", "read")
write = require_module("ai", "write")


# ---- Setup: season config (ERP-owned) + pipeline triggers (proxied) ----
@router.get("/season-config")
def get_season_config(principal: Principal = Depends(read),
                      db: Session = Depends(tenant_db)):
    return service.get_season_config(db)


@router.post("/season-config")
def save_season_config(payload: SeasonConfig, principal: Principal = Depends(write),
                       db: Session = Depends(tenant_db)):
    return service.save_season_config(db, principal.tenant_id, payload.model_dump())


@router.get("/ingest/status")
def ingest_status(principal: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.ingest_status(db, principal.tenant_id)


@router.post("/ingest/sync")
def ingest_sync(principal: Principal = Depends(write), db: Session = Depends(tenant_db)):
    return service.run_ingest(db, principal.tenant_id)


@router.get("/forecast/job")
def forecast_job(principal: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.forecast_job(db, principal.tenant_id)


@router.post("/forecast/refresh")
def forecast_refresh(principal: Principal = Depends(write), db: Session = Depends(tenant_db)):
    return service.refresh_forecast(db, principal.tenant_id)


@router.get("/forecast/backtest-status")
def backtest_status(principal: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.backtest_status(db, principal.tenant_id)


@router.post("/forecast/backtest")
def run_backtest(principal: Principal = Depends(write), db: Session = Depends(tenant_db)):
    return service.run_backtest(db, principal.tenant_id)


# ---- Reads ----
@router.get("/dashboard")
def dashboard(principal: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.dashboard(db, principal.tenant_id)


@router.get("/products")
def products(principal: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.products(db, principal.tenant_id)


@router.get("/projections")
def projections(principal: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.projections(db, principal.tenant_id)


@router.get("/projections/{reference}/detail")
def projection_detail(reference: str, principal: Principal = Depends(read),
                      db: Session = Depends(tenant_db)):
    return service.projection_detail(db, principal.tenant_id, reference)


@router.get("/budget")
def budget(scenario: str = Query("base"), principal: Principal = Depends(read),
           db: Session = Depends(tenant_db)):
    return service.budget(db, principal.tenant_id, scenario)


@router.get("/recommendations")
def recommendations(scenario: str = Query("base"), principal: Principal = Depends(read),
                    db: Session = Depends(tenant_db)):
    return service.recommendations(db, principal.tenant_id, scenario)


@router.get("/validation")
def validation(scenario: str = Query("base"), principal: Principal = Depends(read),
               db: Session = Depends(tenant_db)):
    return service.validation(db, principal.tenant_id, scenario)


@router.get("/customers")
def customers(principal: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.customers(db, principal.tenant_id)


@router.get("/customer-detail")
def customer_detail(customer: str = Query(...), principal: Principal = Depends(read),
                    db: Session = Depends(tenant_db)):
    return service.customer_detail(db, principal.tenant_id, customer)


@router.get("/audit")
def audit(limit: int = Query(30, le=200), principal: Principal = Depends(read),
          db: Session = Depends(tenant_db)):
    return service.audit(db, principal.tenant_id, limit)


@router.get("/forecast/accuracy")
def forecast_accuracy(principal: Principal = Depends(read),
                      db: Session = Depends(tenant_db)):
...
def revert_override(payload: RevertPayload, principal: Principal = Depends(write),
                    db: Session = Depends(tenant_db)):
    return service.revert_override(db, payload.reference, payload.couleur, payload.taille)


@router.post("/overrides/revert-all")
def revert_all(principal: Principal = Depends(write), db: Session = Depends(tenant_db)):
    return service.revert_all_overrides(db)


# ---- Sync ----
@router.post("/sync")
def sync(principal: Principal = Depends(write), db: Session = Depends(tenant_db)):
    return service.sync_all(db, principal.tenant_id)


@router.get("/sync-state")
def get_sync_state(principal: Principal = Depends(read), db: Session = Depends(tenant_db)):
