...
# Every screen in this router is part of the Admin module, which the Roles
# matrix reserves for Admin/Super Admin only (nobody else holds admin.users).
# Previously these endpoints only required tenant membership — any signed-in
# user could call them directly even though the module is hidden from their
# sidebar. Gate them all behind the same permission the team-management
# endpoints in auth/router.py already use, for one consistent check.
...
def users_screen(limit: int = Query(50, le=200), offset: int = Query(0, ge=0),
                 _: Principal = Depends(require_admin), db: Session = Depends(tenant_db)):
    return service.users_screen(db, limit=limit, offset=offset)


@router.post("/users", status_code=status.HTTP_201_CREATED)
def create_user(payload: UserCreate, principal: Principal = Depends(require_admin),
...
def create_role(payload: RoleCreate, principal: Principal = Depends(require_admin),
                db: Session = Depends(tenant_db)):
...
def create_rule(payload: ApprovalRuleCreate, principal: Principal = Depends(require_admin),
                db: Session = Depends(tenant_db)):
...
def audit_screen(limit: int = Query(100, le=500),
                 _: Principal = Depends(require_admin), db: Session = Depends(tenant_db)):
