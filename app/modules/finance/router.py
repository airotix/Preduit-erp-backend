...
from app.modules.finance.dto import (
    ...
    LedgerPaymentTypeIn, PaymentCreate, PaymentUpdate, PeriodCreate,
)

router = APIRouter(prefix="/finance", tags=["finance"])

read = require_module("finance", "read")
write = require_module("finance", "write")
...
@router.get("/fx/rates")
def fx_rates(_: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return fx_service.rates_screen(db)


@router.post("/fx/sync-rates")
def fx_sync_rates(principal: Principal = Depends(write), db: Session = Depends(tenant_db)):
    return fx_service.sync_rates(db, principal.tenant_id)


@router.get("/accounts")
def finance_accounts(_: Principal = Depends(read), db: Session = Depends(tenant_db)):
...
# ---- Period close ----
@router.get("/periods")
def periods(_: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.periods_screen(db)


@router.post("/periods", status_code=status.HTTP_201_CREATED)
def create_period(payload: PeriodCreate, principal: Principal = Depends(write),
                  db: Session = Depends(tenant_db)):
...
def close_period(public_id: str, principal: Principal = Depends(write),
                 db: Session = Depends(tenant_db)):
...
def reopen_period(public_id: str, principal: Principal = Depends(write),
                  db: Session = Depends(tenant_db)):
...
def create_budget(payload: BudgetLineCreate, principal: Principal = Depends(write),
                  db: Session = Depends(tenant_db)):
...
def fixed_assets(currency: str = Query("PKR"), _: Principal = Depends(read), db: Session = Depends(tenant_db)):
...
def create_fixed_asset(payload: FixedAssetCreate, principal: Principal = Depends(write),
                       db: Session = Depends(tenant_db)):
...
def depreciate(public_id: str, principal: Principal = Depends(write),
               db: Session = Depends(tenant_db)):
...
def vat_return(currency: str = Query("PKR"), principal: Principal = Depends(read),
               db: Session = Depends(tenant_db)):
...
def bank_accounts(_: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.bank_screen(db)


@router.post("/bank-accounts", status_code=status.HTTP_201_CREATED)
def create_bank_account(payload: BankAccountCreate, principal: Principal = Depends(write),
                        db: Session = Depends(tenant_db)):
...
def auto_match(public_id: str, principal: Principal = Depends(write),
               db: Session = Depends(tenant_db)):
...
def reconcile(public_id: str, principal: Principal = Depends(write),
              db: Session = Depends(tenant_db)):
...
def create_journal_entry(payload: JournalFull, principal: Principal = Depends(write),
                         db: Session = Depends(tenant_db)):
...
def reverse_journal_entry(public_id: str, principal: Principal = Depends(write),
                          db: Session = Depends(tenant_db)):
...
@router.post("/gl/sync")
def gl_sync(principal: Principal = Depends(write), db: Session = Depends(tenant_db)):
    posted = service.post_unposted(db, tenant_id=principal.tenant_id)
    return {"posted": posted}


@router.get("/customer-ledger")
def customer_ledger(currency: str = Query("PKR"), _: Principal = Depends(read), db: Session = Depends(tenant_db)):
...
@router.put("/ledger-entries/payment-type")
def update_ledger_payment_type(payload: LedgerPaymentTypeIn,
                               principal: Principal = Depends(write),
                               db: Session = Depends(tenant_db)):
    ok = service.update_ledger_payment_type(
        db, edit_type=payload.type, public_id=payload.publicId, payment_type=payload.paymentType)
    if not ok:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Ledger row not found")
    return {"ok": True}
...
def cash_ledger(currency: str = Query("PKR"), _: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.convert(db, service.cash_bank_ledger(db, kind="cash"), currency=currency)


@router.get("/bank-ledger")
def bank_ledger(currency: str = Query("PKR"), _: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.convert(db, service.cash_bank_ledger(db, kind="bank"), currency=currency)


@router.post("/cash-ledger/entries", status_code=status.HTTP_201_CREATED)
def create_cash_ledger_entry(payload: LedgerEntryIn, principal: Principal = Depends(write),
                             db: Session = Depends(tenant_db)):
    le = service.create_cash_bank_entry(
        db, tenant_id=principal.tenant_id, kind="cash",
        description=payload.description, debit=payload.debit, credit=payload.credit)
    return {"public_id": str(le.public_id)}


@router.post("/bank-ledger/entries", status_code=status.HTTP_201_CREATED)
def create_bank_ledger_entry(payload: LedgerEntryIn, principal: Principal = Depends(write),
                             db: Session = Depends(tenant_db)):
    le = service.create_cash_bank_entry(
        db, tenant_id=principal.tenant_id, kind="bank",
        description=payload.description, debit=payload.debit, credit=payload.credit)
    return {"public_id": str(le.public_id)}


@router.get("/profitability")
def finance_profitability(currency: str = Query("PKR"), _: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.convert(db, service.profitability_screen(db), currency=currency)


@router.get("/supplier-ledger")
def supplier_ledger(currency: str = Query("PKR"), _: Principal = Depends(read), db: Session = Depends(tenant_db)):
...
def create_account(payload: AccountCreate, principal: Principal = Depends(write),
                   db: Session = Depends(tenant_db)):
...
def create_journal(payload: JournalEntryCreate, principal: Principal = Depends(write),
                   db: Session = Depends(tenant_db)):
...
def journal_detail(public_id: str, _: Principal = Depends(read), db: Session = Depends(tenant_db)):
...
def create_payment(payload: PaymentCreate, principal: Principal = Depends(write),
                   db: Session = Depends(tenant_db)):
...
def araging_screen(_: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.araging_screen(db)


@router.post("/araging/send-reminders")
def send_reminders(principal: Principal = Depends(write),
                   db: Session = Depends(tenant_db)):
    return service.send_ar_reminders(db, tenant_id=principal.tenant_id)


@router.get("/apaging/screen")
def apaging_screen(_: Principal = Depends(read), db: Session = Depends(tenant_db)):
    return service.apaging_screen(db)


@router.post("/apaging/schedule-payments")
def schedule_payments(principal: Principal = Depends(write),
                      db: Session = Depends(tenant_db)):
...
def create_bill(payload: BillCreate, principal: Principal = Depends(write),
                db: Session = Depends(tenant_db)):
